...
export default function SkillsClient({ initial }: Readonly<{ initial: Skill[] }>) {
  ...
  const openEdit = (s: Skill) => { setForm({ ...s }); setOriginalName(s.name); };
  ...
  const save = async () => {
    ...
    setSkills((prev) => isEdit ? prev.map((s) => s.name === originalName ? saved : s) : [...prev, saved]);
    setForm(null);
  ...
  const confirmRowDelete = async () => {
    ...
    const res = await fetch(`/api/admin/skills?name=${encodeURIComponent(deleteName)}`, { method: "DELETE" });
  ...
  const confirmBulkDelete = async () => {
    ...
    const succeeded = await runBulk(names, (name) =>
      fetch(`/api/admin/skills?name=${encodeURIComponent(name)}`, { method: "DELETE" })
    );
    setSkills((prev) => prev.filter((s) => !succeeded.includes(s.name)));
    clearSelection();
    setBulkDeleteConfirm(false);
    const failed = names.length - succeeded.length;
    showToast(failed > 0 ? `deleted ${succeeded.length}, ${failed} failed` : `deleted ${succeeded.length} skills`, failed > 0 ? "error" : "success");
  };
  ...
  const applyBulkEdit = async () => {
    ...
    const succeeded = await runBulk(targets, (s) =>
      fetch("/api/admin/skills", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...s, ...patch }) })
    );
    const succeededNames = new Set(succeeded.map((s) => s.name));
    setSkills((prev) => prev.map((s) => succeededNames.has(s.name) ? { ...s, ...patch } : s));
    clearSelection();
    setBulkEdit(false);
    setBulkProficiency("");
    setBulkApplyPct(false);
    const failed = targets.length - succeeded.length;
    showToast(failed > 0 ? `updated ${succeeded.length}, ${failed} failed` : `updated ${succeeded.length} skills`, failed > 0 ? "error" : "success");
  };
    ...
    <div className="pb-16">
      ...
      <div className="border border-border overflow-hidden">
        <table className="w-full text-sm font-mono">
          ...
          <tbody>
            {skills.map((s) => (
              <tr key={s.name} className={`border-b border-border hover:bg-surface/50 ${selected.has(s.name) ? "bg-accent/5" : ""}`}>
                <td className="px-4 py-3"><input type="checkbox" checked={selected.has(s.name)} onChange={() => toggleOne(s.name)} className="accent-[#00ff9f]" /></td>
                <td className="px-4 py-3 text-text-primary">{s.name}</td>
                <td className="px-4 py-3 text-muted text-xs">{s.proficiency}</td>
                <td className="px-4 py-3 w-48"><SkillBar percentage={s.percentage} /></td>
                <td className="px-4 py-3 text-accent text-xs">{s.percentage}%</td>
                <td className="px-4 py-3">
                  <div className="flex gap-3 justify-end">
                    <button onClick={() => openEdit(s)} className="text-muted hover:text-accent"><Pencil size={13} /></button>
                    <button onClick={() => setDeleteName(s.name)} className="text-muted hover:text-red-400"><Trash2 size={13} /></button>
                  </div>
                </td>
              </tr>
            ))}
            {skills.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-muted text-xs">no skills yet</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {form && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-6">
          <div className="bg-surface border border-border w-full max-w-sm p-6 font-mono">
            <h2 className="text-accent text-sm mb-4">{originalName ? "edit skill" : "add skill"}</h2>
      ...
      {bulkEdit && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-6">
          <div className="bg-surface border border-border w-full max-w-sm p-6 font-mono">
            ...
            <p className="text-xs text-muted mb-4">{selected.size} skills selected</p>
            <div className="space-y-4">
              <div>
                <label className="block text-xs text-muted mb-1">proficiency</label>
      ...
      {bulkDeleteConfirm && <ConfirmModal message={`delete ${selected.size} skills?`} onConfirm={confirmBulkDelete} onCancel={() => setBulkDeleteConfirm(false)} />}
