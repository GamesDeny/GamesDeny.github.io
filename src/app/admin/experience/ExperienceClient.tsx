...
export default function ExperienceClient({
  ...
  const allSelected =
    activeIds.length > 0 && activeIds.every((id) => activeSelected.has(id));
  ...
  const toggleAll = () =>
    setActiveSelected(allSelected ? new Set() : new Set(activeIds));
  ...
  const saveWork = async () => {
    ...
    setExperience((prev) =>
      isEdit
        ? prev.map((e) => (e.id === workForm.id ? saved : e))
        : [...prev, saved],
    );
    setWorkForm(null);
  ...
  const saveEdu = async () => {
    ...
    setEducation((prev) =>
      isEdit
        ? prev.map((e) => (e.id === eduForm.id ? saved : e))
        : [...prev, saved],
    );
    setEduForm(null);
    showToast(isEdit ? "entry updated" : "entry created");
  ...
  const confirmRowDelete = async () => {
    ...
    const url =
      type === "work"
        ? `/api/admin/experience?id=${id}`
        : `/api/admin/education?id=${id}`;
  ...
  const confirmBulkDelete = async () => {
    ...
    const endpoint =
      tab === "work" ? "/api/admin/experience" : "/api/admin/education";
    const succeeded = await runBulk(ids, (id) =>
      fetch(`${endpoint}?id=${id}`, { method: "DELETE" }),
    );
    if (tab === "work")
      setExperience((prev) => prev.filter((e) => !succeeded.includes(e.id!)));
    ...
    showToast(
      failed > 0
        ...
        : `deleted ${succeeded.length} entries`,
  ...
  const applyBulkEdit = async () => {
    if (tab === "work") {
      ...
      else if (bulkEndDate === "date" && bulkEndDateVal.trim())
        patch.endDate = bulkEndDateVal.trim();
  ...
  function TagPicker({
    ...
    readonly extraTags: string[]; // tags on the entry that aren't in allTags (typed manually in the past)
  }) {
    const display = Array.from(new Set([...allTags, ...extraTags])).sort(
      ...
      <div className="flex flex-wrap gap-2 p-3 border border-border bg-background min-h-[48px]">
        ...
        {display.length === 0 && (
          <span className="text-xs text-muted">
            no tags available — add languages or skills first
          </span>
        )}
      </div>
    );
  ...
  function LocalizedInput({
    ...
    readonly onChange: (v: LocalizedString) => void;
  }) {
    return (
      <div>
        ...
        {locales.map((l) => (
          <div key={l} className="flex items-center gap-2 mb-2">
            <span className="text-xs text-accent w-6 shrink-0 uppercase">
              {l}
            </span>
            <input
              type="text"
  ...
  function LocalizedBullets({
    ...
    readonly label: string;
    readonly value: LocalizedStringArray;
    readonly onChange: (v: LocalizedStringArray) => void;
  }) {
    return (
      <div>
        <label className="block text-xs text-muted mb-2">{label}</label>
                ...
                <div key={i} className="flex gap-2 mb-2">
                  <input
                    ...
                    onChange={(e) =>
                      onChange({
                        ...value,
                        [l]: arr.map((x, j) => (j === i ? e.target.value : x)),
                      })
                    }
                    className="flex-1 bg-background border border-border px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-accent/60"
                  />
                  <button
                    type="button"
                    onClick={() =>
                      onChange({ ...value, [l]: arr.filter((_, j) => j !== i) })
                    }
    ...
    <div className="pb-16">
          ...
          onClick={() =>
            tab === "work"
              ? setWorkForm({
                  id: "",
                  company: "",
                  role: emptyLocalized(locales),
                  startDate: "",
                  endDate: "",
                  location: "",
                  bullets: emptyLocalizedArray(locales),
                  skills: [],
                })
              : setEduForm({
                  id: "",
                  institution: "",
                  degree: emptyLocalized(locales),
                  field: emptyLocalized(locales),
                  startDate: "",
                  endDate: "",
                  gpa: "",
                  notes: emptyLocalizedArray(locales),
                })
          }
          ...
          <table className="w-full text-sm font-mono">
            ...
            <tbody>
              {experience.map((e) => (
                <tr
                  key={e.id}
                  className={`border-b border-border hover:bg-surface/50 ${selectedWork.has(e.id!) ? "bg-accent/5" : ""}`}
                >
                  <td className="px-4 py-3">
                    <input
                      type="checkbox"
                      checked={selectedWork.has(e.id!)}
                  ...
                  <td className="px-4 py-3 text-text-primary">
                    {localized(e.role, displayLocale)}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-3 justify-end">
                      <button
                        type="button"
                        onClick={() => setWorkForm(toWorkForm(e, locales))}
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setDeleteTarget({ type: "work", id: e.id! })
                        }
                        className="text-muted hover:text-red-400"
                      >
                        <Trash2 size={13} />
          ...
          <table className="w-full text-sm font-mono">
            ...
            <tbody>
              {education.map((e) => (
                <tr
                  key={e.id}
                  className={`border-b border-border hover:bg-surface/50 ${selectedEdu.has(e.id!) ? "bg-accent/5" : ""}`}
                >
                  <td className="px-4 py-3">
                    <input
                      type="checkbox"
                      checked={selectedEdu.has(e.id!)}
                      onChange={() => toggleOne(e.id!)}
                      className="accent-[#00ff9f]"
                    />
                  </td>
                  ...
                  <td className="px-4 py-3 text-text-primary">
                    {localized(e.degree, displayLocale)}{" "}
                    {localized(e.field, displayLocale)}
                  </td>
                  <td className="px-4 py-3 text-muted text-xs">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-3 justify-end">
                      <button
                        type="button"
                        onClick={() => setEduForm(toEduForm(e, locales))}
                        className="text-muted hover:text-accent"
                      >
                        <Pencil size={13} />
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          setDeleteTarget({ type: "edu", id: e.id! })
                        }
                        className="text-muted hover:text-red-400"
                      >
                        <Trash2 size={13} />
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-6">
          <div className="bg-surface border border-border w-full max-w-xl p-6 font-mono overflow-y-auto max-h-[90vh]">
            <h2 className="text-accent text-sm mb-4">
              {workForm.id ? "edit work entry" : "new work entry"}
            </h2>
            <div className="space-y-5">
              ...
              <LocalizedInput
                label="role"
                value={workForm.role}
                onChange={(v) => setWorkForm({ ...workForm, role: v })}
              />
              ...
              <LocalizedBullets
                ...
                onChange={(v) => setWorkForm({ ...workForm, bullets: v })}
              />
              ...
              <div>
                ...
                <TagPicker
                  ...
                  extraTags={workForm.skills.filter(
      ...
      {eduForm && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-6">
          <div className="bg-surface border border-border w-full max-w-xl p-6 font-mono overflow-y-auto max-h-[90vh]">
            <h2 className="text-accent text-sm mb-4">
              {eduForm.id ? "edit education entry" : "new education entry"}
            </h2>
            <div className="space-y-5">
              <div>
                <label className="block text-xs text-muted mb-1">
              ...
              <LocalizedInput
                label="field"
                value={eduForm.field}
                onChange={(v) => setEduForm({ ...eduForm, field: v })}
              />
              ...
              <LocalizedBullets
                label="notes"
                value={eduForm.notes}
                onChange={(v) => setEduForm({ ...eduForm, notes: v })}
              />
            </div>
            <div className="flex gap-3 mt-6">
              <button
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-6">
          <div className="bg-surface border border-border w-full max-w-sm p-6 font-mono">
            ...
            <p className="text-xs text-muted mb-4">
              {selectedWork.size} entries selected
            </p>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {(["no-change", "present", "date"] as const).map((opt) => (
                    ...
                    >
                      <input
                        ...
                        name="bulkEndDate"
                        value={opt}
                        checked={bulkEndDate === opt}
                        onChange={() => setBulkEndDate(opt)}
                        className="accent-[#00ff9f]"
                      />
                      {opt === "no-change"
                        ? "— no change —"
                  ...
                  {bulkEndDate === "date" && (
                    <input
                      type="text"
                      value={bulkEndDateVal}
                      onChange={(e) => setBulkEndDateVal(e.target.value)}
      ...
      {bulkEdit && tab === "education" && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-6">
          <div className="bg-surface border border-border w-full max-w-sm p-6 font-mono">
            ...
            <p className="text-xs text-muted mb-4">
              {selectedEdu.size} entries selected
            </p>
            <div>
              <label
                htmlFor="bulk-edu-end-date"
                className="block text-xs text-muted mb-1"
              >
                end date (empty = skip)
              </label>
              <input
                id="bulk-edu-end-date"
                type="text"
                value={bulkEduEndDate}
                onChange={(e) => setBulkEduEndDate(e.target.value)}
      ...
      {bulkDeleteConfirm && (
        <ConfirmModal
          message={`delete ${activeSelected.size} entries?`}
      ...
      {toast && (
        <SaveToast
          ...
          offsetBottom={activeSelected.size > 0}
        />
      )}
